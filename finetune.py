...
Usage: python finetune.py --arch resnet56 --dataset cifar10 --sparsity_level 0.4 --action_num 5
"""
...
from decision import default_graph, apply_func, set_deterministic_value, set_pruning_threshold
import misc
...
optimizer = torch.optim.SGD(
    model_params, lr=args.lr, momentum=args.mm, weight_decay=args.wd,
)
...
for epoch in range(args.epochs):
    ...
    if acc > best_acc:
        ...
        print("New best @ Epoch %d, Accuracy = %.4f, Sparsity = %.4f — checkpoint saved\n"
              % (epoch, acc, sparsity))
    else:
        print("Epoch %d, Accuracy = %.4f, Sparsity = %.4f (best = %.4f)\n"
